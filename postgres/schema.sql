...
create table if not exists blacklist_sections (
...
create index if not exists blacklist_entries_guild_category_idx
    on blacklist_entries (guild_id, category, position);

create table if not exists blacklist_message_refs (
    guild_id text not null references blacklist_guilds (guild_id) on delete cascade,
    category text not null,
...
create table if not exists blacklist_named_documents (
    guild_id text not null references blacklist_guilds (guild_id) on delete cascade,
